...
for (const d of diameters) {
    ...
    let dnum = d.replace('cm', '');
    
    // Some lines need to be specifically adjusted for the current diameter variables
    // Replace the fallback `priceMap['m30'] || m30` with the correct ones
    ...
    // Add missing manual price string for meta descriptions if not present
    // actually just use `manualPrice || ""` 
    out = out.replace(/\.replace\('\{manual\}', manualPrice\)/g, `.replace('{manual}', manualPrice || '')`);
    
    // Add `.replace` to costCalc in the template if not already there
    ...
    // Add `.replace` to FAQ in JSON-LD
    // plainFaqSchema mapping is:
    // q: item.q.replace(/<[^>]*>?/gm, ''),
    // a: item.a.replace(/<[^>]*>?/gm, '')
    ...
    // Add `.replace` to FAQ HTML rendering
    ...
    // We also need to conditionally render `project.total`, `project.specs`, `project.time`, `project.mobilization`, `project.title`, and `project.costCalc` if they don't exist, due to imageOnly projects!
    // We can replace the whole project-text block:
    ...
    // Also the tables need to conditionally hide manual if `showManualHighlight` is false?
    // In `recovered_30cm.astro` we had hardcoded tables.
    // Replace the tables block with dynamic generic tables
...
`;
    ...
    // Also inject priceRange handling to the localBusinessSchema!
    out = out.replace(/"description": pageData\.localBusinessDescTemplate[^,]+,/, `$&
